...
from baseservice.async_service.async_server_loop_service import AsyncServerLoopService
from baseservice.async_service.iodevices.base import (AsyncInputTransactionScope,
                                                      AsyncInputDeviceManager,
                                                      AsyncAggregateInputDevice)
from baseservice.iodevices.base import InputDevice, Message, DeviceHeaders


class AsyncDeviceReaderService(AsyncServerLoopService, metaclass=ABCMeta):
    def __init__(self, *,
                 input_device_manager: AsyncInputDeviceManager,
        ...
        self._aggregate_input_device: Optional[AsyncAggregateInputDevice] = None

    async def _async_prepare_service(self):
        await self._input_device_manager.connect()
        self._aggregate_input_device = await self._input_device_manager.get_aggregate_device(self._input_device_names)

    async def _async_server_loop(self, cancellation_token: asyncio.Event):
        async with AsyncInputTransactionScope(device=self._aggregate_input_device,
                                              with_transaction=self._use_transactions) as transaction_scope:
            batch = []

            # read first message with _read_timeout anyway
            message, device_headers = await transaction_scope.read_message(timeout=self._read_timeout)
            ...
            for i in range(self._max_batch_read_count - 1):  # try to read the rest of the batch
                ...
                message, device_headers = await transaction_scope.read_message(timeout=timeout)
                if message is None:
            ...
            if batch:
                await self._handle_messages(batch)

            transaction_scope.commit()

    async def _async_finalize_service(self, exception: Optional[Exception] = None):
        await self._input_device_manager.disconnect()

    @abstractmethod
    async def _handle_messages(self, batch: List[Tuple[InputDevice, Message, DeviceHeaders]]):
        pass
